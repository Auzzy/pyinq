...
.. function:: assert_equal(actual, expected)
   ...
   Test the equivalence of ``actual`` and ``expected``. Although not enforced, ``actual`` should be the result of the expression under test and ``expected`` should be the passing value. Equivalence is determined by the == operator (equality by value).
...
.. function:: assert_is_instance(obj, cls)
   ...
   Tests whether ``obj`` is an instance of ``cls``. Just as with :func:`isinstance`, ``cls`` can be a tuple of classes.

.. function:: assert_attrib(obj, attrib_name)
   assert_not_attrib(obj, attrib_name)
   eval_attrib(obj, attrib_name)
   eval_not_attrib(obj, attrib_name)

   Tests whether ``obj`` contains an attribute with the name ``attrib_name``. Note that in Python, an attribute is any member of the obj, no matter the type: field, function, nested class, built-in value, etc.
